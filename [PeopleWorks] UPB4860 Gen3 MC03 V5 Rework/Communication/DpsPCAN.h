...
#define	PCAN_MAX_HANDLES				59
#define PCAN_MAX_MSG_BYTE				8
#define PCAN_MAX_MSG_BYTE_FD			64

#define PCAN_MAX_RCVD_DATA_BUFFER		512
#define PCAN_MAX_RCVD_DATA_SIZE			4096
#define PCAN_MAX_DATA_BUFFER_SIZE		20

#define PCAN_MAX_RETRY_COUNT			3
...
///////////////////////////////////////////////////////////////////////////////
// 비트레이트 정의 ////////////////////////////////////////////////////////////
#define PCAN_BR_CLOCK					_T("f_clock")
#define PCAN_BR_CLOCK_MHZ				_T("f_clock_mhz")
#define PCAN_BR_NOM_BRP					_T("nom_brp")
#define PCAN_BR_NOM_TSEG1				_T("nom_tseg1")
#define PCAN_BR_NOM_TSEG2				_T("nom_tseg2")
#define PCAN_BR_NOM_SJW					_T("nom_sjw")
#define PCAN_BR_NOM_SAMPLE				_T("nom_sam")
#define PCAN_BR_DATA_BRP				_T("data_brp")
#define PCAN_BR_DATA_TSEG1				_T("data_tseg1")
#define PCAN_BR_DATA_TSEG2				_T("data_tseg2")
#define PCAN_BR_DATA_SJW				_T("data_sjw")
#define PCAN_BR_DATA_SAMPLE				_T("data_ssp_offset")
...
///////////////////////////////////////////////////////////////////////////////
// 사용자 메세지 //////////////////////////////////////////////////////////////
#define UM_DPS_DEV_PCAN_RCVD_DATA		(WM_DPS_DEV_PCAN)
#define UM_DPS_DEV_PCAN_SEND_DATA		(WM_DPS_DEV_PCAN + 1)
#define UM_DPS_DEV_PCAN_DEVICE_REMOVED	(WM_DPS_DEV_PCAN + 2)
...
typedef struct tagTPCANMsg
{
    ...
    BYTE					DATA[PCAN_MAX_MSG_BYTE];				// Data of the message (DATA[0]..DATA[7])
...
} TPCANTimestamp, *PTPCANTimestamp;
...
typedef struct tagTPCANMsgFD
{
    ...
    BYTE					DATA[PCAN_MAX_MSG_BYTE_FD];				// Data of the message (DATA[0]..DATA[63])

} TPCANMsgFD, *PTPCANMsgFD;
...
typedef struct {
	...
	TCHAR					szModelName[MAX_STRINPUT_S];
	TCHAR					szSerialNo[MAX_STRINPUT_S];
	TCHAR					szVersion[MAX_STRINPUT_S];

} PCAN_DEVICE_INFO_T, *PPCAN_DEVICE_INFO_T;

// 디바이스 연결 정보
typedef struct {

	TPCANHandle				hHandle;
	TPCANBaudrate			wBaudrate;
	TPCANType				nTypeHW;
	DWORD					dwSelectIO;
	WORD					wInterrupt;
	BOOL					bIsFD;

} PCAN_CONNECT_INFO_T, *PPCAN_CONNECT_INFO_T;

// 라이브러리 정보
typedef struct {

	DWORD					dwDeviceCount;
	PCAN_DEVICE_INFO_T		stDeviceInfo[PCAN_MAX_HANDLES];

	TCHAR					szVersionAPI[MAX_STRINPUT_S];
	
} PCAN_LIB_INFO_T, *PPCAN_LIB_INFO_T;

// CAN 패킷 구조체
typedef struct {

	TPCANMsgFD				stMessage;
	TPCANTimestamp			stTimeStamp;
	TPCANTimestampFD		nTimeStampFD;

} PCAN_PACKET_T, *PPCAN_PACKET_T;
...
// CDpsPCAN 클래스 ////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////
class AFX_EXT_CLASS CDpsPCAN : public CWnd
{
	DECLARE_DYNAMIC(CDpsPCAN)

///////////////////////////////////////////////////////////////////////////////
// 클래스 멤버 ////////////////////////////////////////////////////////////////
public:
	static	HINSTANCE				m_hLibHandle;
	static	CCriticalSection		m_csDeviceReset;
	static	PCAN_LIB_INFO_T			m_stLibraryInfo;
	static	TPCANHandle				m_hHandleArray[PCAN_MAX_HANDLES];

public:
	static	BOOL					PCAN_LoadLibrary(void);
	static	BOOL					PCAN_FreeLibrary(void);
	static	BOOL					PCAN_FindDevice(void);

	static	PCAN_LIB_INFO_T			PCAN_GetLibraryInfo(void)					{ return CDpsPCAN::m_stLibraryInfo; }
	static	PCAN_DEVICE_INFO_T		PCAN_GetChannelName(TPCANHandle hHandle, BOOL bIsFD);
	static	CString					PCAN_GetHandleName(TPCANHandle hHandle);
	static	CString					PCAN_GetTypeMsgString(TPCANMessageType nType);
...
protected:
	...
	PCAN_PACKET_T					m_stPacketBuffer[PCAN_MAX_RCVD_DATA_BUFFER];
	UINT							m_nPosHead;									// CAN 수신 버퍼 헤드 인덱스
	UINT							m_nPosTail;									// CAN 수신 버퍼 테일 인덱스

	PCAN_DEVICE_INFO_T				m_stDeviceInfo;

	BOOL							m_bIsDeviceConnected;

	CCriticalSection				m_csRcvdBuffer;
	LTimeHighResCounter				m_LTimeCounter;

public:
	void							SetMonitorWnd(CWnd* pWnd)					{ m_pMonitorWnd = pWnd; }
	void							ResetTimeCounter(void)						{ m_LTimeCounter.Reset(); }
...
protected:
	...
	CCriticalSection				m_csHoldThread;
	CCriticalSection				m_csProceed;
	volatile BOOL					m_bExitThread;
	volatile BOOL					m_bHoldThread;

	static UINT						ThreadFuncMonitor(LPVOID lpParam);


///////////////////////////////////////////////////////////////////////////////
// 사용자 함수 ////////////////////////////////////////////////////////////////
public:
	BOOL							DeviceOpen(PCAN_CONNECT_INFO_T stConnectInfo, PPCAN_DEVICE_INFO_T pstDeviceInfo = NULL, CWnd* pParentWnd = NULL);
	BOOL							DeviceReset(BOOL bHardReset = TRUE);
	void							DeviceClose(void);
	void							ClearBuffer(void);

	BOOL							SendPacket(PCAN_PACKET_T& stPacket);
	BOOL							ReadPacket(PCAN_PACKET_T& stPacket);

	BOOL							SetMaskCAN_ID(BOOL bExtMode, INT32U nCAN_ID);


///////////////////////////////////////////////////////////////////////////////
// 디바이스 함수 //////////////////////////////////////////////////////////////
protected:
	TPCANStatus						PCAN_SendMessage(PCAN_PACKET_T& stPacket);
	TPCANStatus						PCAN_SendMessageFD(PCAN_PACKET_T& stPacket);

	TPCANStatus						PCAN_ReadMessage(void);
	TPCANStatus						PCAN_ReadMessageFD(void);
	void							PCAN_ReadMessages(void);

	void							PCAN_ProcessMessage(TPCANMsg stCANMsg, TPCANTimestamp stCANTimeStamp);
	void							PCAN_ProcessMessageFD(TPCANMsgFD stCANMsg, TPCANTimestampFD nCANTimeStamp);


///////////////////////////////////////////////////////////////////////////////
// 클래스 기본 ////////////////////////////////////////////////////////////////
public:
	CDpsPCAN(CWnd* pParent = NULL);
	virtual							~CDpsPCAN();
...
public:
	...
	afx_msg	LRESULT					OnMsgReceivedData(WPARAM wParam, LPARAM lParam);
	afx_msg	LRESULT					OnMsgDeviceRemoved(WPARAM wParam, LPARAM lParam);
};
